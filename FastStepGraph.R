...
FastStepGraph = function(x, alpha_f, alpha_b=0, nei.max=0, data_scale=FALSE){
    ...
    f_ij = cor(e)
    f_ij = abs(f_ij[lower.tri(f_ij)]) 
    b_ij = rep(2, length(f_ij))
    
    k = 1
    K = 2*length(Edges_I[,2]) # length(Edges_I[,2]) is the total number of edges
    while ((k <= K)) {
        ...
        # of neighbors (nei.max), refrain from adding a new neighbor to that node.
            ...
            if (b_ij_min <= alpha_b){
                ...
                b_ij[b_ij_indx] = 2
                H = which((Edges_I[,2]==i_f) | (Edges_I[,2]==j_f) | (Edges_I[,1]==i_f) | (Edges_I[,1]==j_f) | (Edges_I[,2]==i_b) | (Edges_I[,2]==j_b) | (Edges_I[,1]==i_b) | (Edges_I[,1]==j_b))
            }
            else { H = which((Edges_I[,2]==i_f) | (Edges_I[,2]==j_f) | (Edges_I[,1]==i_f) | (Edges_I[,1]==j_f)) }
           
            ################# Update Correlation coefficients ##################
            cor_matrix = cor(e)
            call = cbind(Edges_I[H, 1], Edges_I[H, 2])
            f_ij[H] = abs(cor_matrix[call])
            f_ij[f_ij_indx] = 0
